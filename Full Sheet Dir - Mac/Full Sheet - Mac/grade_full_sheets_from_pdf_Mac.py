...
def image_to_grid(img, student):
    ...
    dark_pixel_cutoff = 125  # Grayscale value below which a pixel is considered dark
...
def rotate_image(image, student, log_file):
    ...
    # Hue range for green is approximately 35 to 85.
    ...
    # cv2.imshow('Green Range to White', image)
    # cv2.waitKey(0)
    ...
    for cnt in contours:
        ...
        if 600 < area: # for the small rectangle markers. there are 61 total
            x, y, w, h = cv2.boundingRect(cnt)
            if x < 170: # we need to ignore any dark over-filled bubbles
    ...
    # Proceed only if 61 contours are found
...
def crop_to_answers(image, student, log_file):
    ...
    if  1 < len(selected_contours) <= 61:
        ...
        x_start, y_start, x_end, y_end  = x_corners[0] + 107, y_corners[0] + 285, x_corners[-1] + 2370, y_corners[-1] + 35
        ...
        x_start, y_start, x_end, y_end = 220, 365, 2485, 3120
        image = image[y_start:y_end, x_start:x_end]
...
def process_scanned_page(image, student, log_file): 
    ...
    try:
        # ID Number
        test_form_grid = numpy_array_values[8:18, 29:38]
    ...
    for column in test_form_grid.T:
        ...
        if len(index) > 0:
            # print(index[0])
            last_name.append(index_to_letter(index[0]))
    last_name = ''.join(map(str, last_name))
    # print(last_name)

    # First Name
    test_form_grid = numpy_array_values[2:27, 16:25]
    for column in test_form_grid.T:
        index = np.where(column)[0]
        if len(index) > 0:
            first_name.append(index_to_letter(index[0]))
    first_name = ''.join(map(str, first_name))
    # print(first_name)

    # Test Form Number
    test_form_grid = numpy_array_values[23:32, 31:33]
    # print(test_form_grid)
    # for column in test_form_grid.T:
    #     print(f"Column: {column}")
        # test_num.append(np.where(column))
    ...
    question_iterator = 1
    column_iterator = 0
    for column_iterator in range(5):
...
def main():
        ...
        if ak_filename.lower().endswith('.csv'): 
            ...
            with open(answer_key_path, 'r', newline='') as answer_key:
                ...
                for index, row_dict in enumerate(csv_dict_reader, start=0):
                    ...
                    if row_dict['Form 5']:
                        ANSWER_KEY_5[index] = CHOICE_INDEX.index(row_dict['Form 5'])
                    if row_dict['Points']:
    ...
    with open(output_csv, 'w', newline='') as f: 
        ...
        header_list = ["Scanned Page", "ID", "LastName", "FirstName", "Form", "Score"]
        ...
        for filename in sorted(os.listdir(input_dir)): 
            ...
            # try: 
                ...
                for i, page_image_pil in enumerate(pages_as_images): 
                    ...
                    student_row = [student, exam_results[2], exam_results[4], exam_results[5],  exam_results[3], exam_results[0]]
